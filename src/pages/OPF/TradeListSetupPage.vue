...
export default {
  name: 'TradeListSetupPage',
  setup () {
    const router    = useRouter()
    const store     = useStore()
    const page      = usePage({title: 'Trade List: Setup OPF'})
    const columns   = ref({})

    onMounted(async () => {
      await store.dispatch('loadTradeSetupOPF')
      columns.value         = store.state.tradeSetupOPF.columns
      page.isLoading.value  = store.state.tradeSetupOPF.isLoading
    })
